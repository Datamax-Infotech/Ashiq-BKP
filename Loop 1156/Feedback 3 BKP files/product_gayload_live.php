...
<div class="main-section mb-5 pb-5">
    ...
    <div class="dashboard_heading" style="font-size: 24px;font-family: 'Titillium Web', sans-serif;font-weight: 600;padding: 1% 0;">
				B2B Inventory: Used Gaylord Totes
		</div>
		<div class="row">
				...
				<div class="row">
					...
					<div class="form-group">
						...
						<table class="form-table">
							<tr>
								<td><input name="min_height" type="number" class="w-100" value="0" min="0" max="99" id="height-from" onblur="show_inventories('',1)"/></td>
								<td>&nbsp;-&nbsp;</td>
								<td><input name="max_height" type="number" class="w-100" value="99" min="0" max="99" id="height-to" onblur="show_inventories('',1)"/></td>
							</tr>
						</table>
						<!--<div class="row">
				...
				if( !$.isArray(result) ||  result.length ==0 ) {
					...
					if(view_type=="list_view"){
						...
						$.each(result,function(index,res){
							...
							result_str +='<div class="col-md-2 align-self-center">';
					...
					$.each(result,function(index,res){
						...
						result_str +=`<tr>	
										<td width='50px' id="after_po${index}">
											<a href="javascript:void(0)" onclick="display_preoder_sel(${index}, ${res['reccnt']}, ${res['loop_id']}, ${res['box_warehouse_id']})"><u>${res['colorvalueQty']}</u></a>
										</td>
										<td width='100px'>${res['lead_time_of_FTL']}</td>
										<td width='50px'>${res['percent_per_load']}%</td>
										<td width='50px'>$${res["minfob"]}</td>
										<td width='50px'>${res['b2b_id']}</td>
										<td width='50px'>${res["distance"]}</td>
										<td width='100px'>${res["status"]}</td>
										<td class="text-left" width='300px'><a class="text-primary" onmouseover="Tip('${res['description_hover_notes']}')"  onmouseout="UnTip()" target="_blank" href="manage_box_b2bloop.php?id=${res['loop_id']}&proc=View&"><u>${res['system_description']}</u></a></td>
										<td class="text-left" width='150px'>${res['supplier_name']}</td>
										<td class="text-left" width='150px'>${res["shipFromLocation"]}</td>
									</tr>`;
