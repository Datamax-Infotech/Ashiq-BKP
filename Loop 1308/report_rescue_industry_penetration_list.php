...
<div class="main_data_css">
	...
	<? if ($cron_rep_flg == "no") { ?>
		<div class="dashboard_heading" style="float: left;">
			...
			<span class="tooltiptext">This report shows the user all of the largest companies within each industry which generally is qualified to sell to UCB (Purchasing Records). If you are looking for industries which buy from UCB, then use the Industry Penetrations Report - Sales Records.</span></div><br>
			</div>
		</div>
	<? } ?>
	...
	Note: Wait for <font color="red">Report</font> to complete, use the Sort option after the Report is completed. Rescue records are shown in <span style="background:#bcf5bc;">Green</span> </i></div>
	<? } ?>
	<? 
		...
		function getreportdata($eid, $so_val, $sk_val, $dt_from, $dt_to, $industry_res)
		{
			...
			<table width="70%" border="0" cellspacing="1" cellpadding="1" class="table_style">
				...
				<tr>
					...
					<td bgcolor="#D9F2FF" align="center"><font size="1" face="Arial, Helvetica, sans-serif" color="#333333">Number Contacted&nbsp;<a href="<? echo $sorturl; ?>&sort_order_pre=ASC&sort=noocontacted"><img src="images/sort_asc.png" width="6px;" height="12px;"></a>&nbsp;<a href="<? echo $sorturl; ?>&sort_order_pre=DESC&sort=noocontacted"><img src="images/sort_desc.png" width="6px;" height="12px;"></a>
		                </font></td>
					<td bgcolor="#D9F2FF" align="center"><font size="1" face="Arial, Helvetica, sans-serif" color="#333333">Number Have Inventory Items&nbsp;<a href="<? echo $sorturl; ?>&sort_order_pre=ASC&sort=haveinv"><img src="images/sort_asc.png" width="6px;" height="12px;"></a>&nbsp;<a href="<? echo $sorturl; ?>&sort_order_pre=DESC&sort=haveinv"><img src="images/sort_desc.png" width="6px;" height="12px;"></a>
		                </font></td>
			...
			?>
				<tr valign="middle">
					<td width="21%" bgcolor="<?=$rowcolor?>"><a target="_blank" href="http://loops.usedcardboardboxes.com/viewCompany.php?ID=<?=$data["I"]?>"><font face="Arial, Helvetica, sans-serif" size="1" color="#333333"><? if ($data["LID"] > 0) echo "<b>"; ?><? echo $nickname; ?><? if ($data["LID"] > 0) echo "</b>"; ?></font></a></td>
					<td width="15%" bgcolor="<?=$rowcolor?>" align="center"><font face="Arial, Helvetica, sans-serif" size="1" color="#333333"><?=$data["EI"]?></font></td>

					<td width="15%" bgcolor="<?=$rowcolor?>" align="center"><font face="Arial, Helvetica, sans-serif" size="1" color="#333333"><?=$industry?></font></td>
					
					<td  bgcolor="<?=$rowcolor?>" align="center"><?=$data["noof_location"]?></td>

					<td width="15%" bgcolor="<?=$rowcolor?>" align="center"><?=$nooflocation?></td>
					<td width="15%" bgcolor="<?=$rowcolor?>" align="center"><?=$nooflocation_notin?></td>
					...
					<td width="10%" bgcolor="<?=$rowcolor?>" align="center">
		                <a href='#' id='<?php echo "inv".$unqid;?>' onclick="load_all_noofinv('<?php echo "inv".$unqid; ?>', '<?=$data["I"]?>','<?php echo $scomp_loopid_list; ?>', '<?php echo $dt_from; ?>', '<?php echo $dt_to; ?>'); return false;"><?=$noofinvitem?></a>
		            </td>
					<td width="10%" bgcolor="<?=$rowcolor?>" align="center">
					...
					<td width="10%" bgcolor="<?=$rowcolor?>" align="center"><?=$loadsold?></td>
					<td width="10%" bgcolor="<?=$rowcolor?>" align="right">$<?=number_format($totrev,2)?></td>
				</tr>
		    ...
		    //
		        ...
		        ?>
		            <tr valign="middle">
		                ...
		                <td width="10%" bgcolor="<?=$rowcolor?>" align="center">
		                    <a href='#' id='<?php echo "inv".$unqid;?>' onclick="load_all_noofinv('<?php echo "inv".$unqid; ?>', '<?=$MGArraytmp2["compid"]?>','<?php echo $scomp_loopid_list; ?>', '<?php echo $dt_from; ?>', '<?php echo $dt_to; ?>'); return false;"><?=$MGArraytmp2["noofinvitem"]?></a>
		                </td>
		                ...
		                <td width="10%" bgcolor="<?=$rowcolor?>" align="center"><?=$MGArraytmp2["loadsold"]?></td>
		                <td width="10%" bgcolor="<?=$rowcolor?>" align="right">$<?=number_format($MGArraytmp2["totrev"],2)?></td>
				</tr>
		        <?
		...
		function getreportdata($eid, $so_val, $sk_val, $dt_from, $dt_to, $industry_res) {
			...
			<table width="900px" border="0" cellspacing="1" cellpadding="1" class="table_style">
				...
				while ($rec_parentrec = array_shift($view_parentrec)) {
					...
					if (tep_db_num_rows($data_res_No_Limit) > 0) {
						...
						while ($data = array_shift($data_res)) {
							...
							$scomp_list = ""; $scomp_loopid_list = "";$scomp_list_l="";$scomp_loopid_list_l="";
							if ($data["I"] != ""){
								/*$sql_child = "SELECT ID, loopid FROM companyInfo where companyInfo.parent_child = 'Child' and parent_comp_id = " . $data["I"] ." OR ID = " . $data["I"];
								//echo $sql_child."<br>";
								$data_result = db_query($sql_child, db_b2b());*/
								//echo "<pre> arrParentChildCI -> "; print_r($arrParentChildCI); echo "</pre>";	
							...
							//echo "<br /> =>".$data["I"]. " / scomp_list - ".$scomp_list." / scomp_loopid_list -> ".$scomp_loopid_list. " / nooflocation -> ".$nooflocation." / nooflocation_notin - ".$nooflocation_notin;
							...
							if ($scomp_list != "") {						
								...
								if ($scomp_loopid_list != ""){
									...
									$resHasInv = db_query($sql_child_HasInv, db_b2b());
						...
						<tr> 
							...
							<td bgcolor="#E4E4E4" align="center"><?=$total_noofinvitem;?></td>
				...
				<tr> 
					...
					<td bgcolor="#D9F2FF" align="center"><?=$gtotal_noofinvitem;?></td>
